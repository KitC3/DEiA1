...
layout = html.Div([
    ...
    html.Div([
    dcc.Markdown('###### Decommission Time'),
    dcc.Input(
            id='decommission_time',
            placeholder="",
            value=11
        ),
    ], style=style),
...
@app.callback(
    ...
    [Input('data_input', 'value'),
     Input('decommission_time', 'value')])
def predict(data_input, decommission_time):
    ...
    if float(decommission_time) > 0:
        difference = ((y_pred - float(decommission_time)) / float(decommission_time)) * 100
        if difference < 0:
            results = f'The predicted decommission time is {y_pred:.2f} years which is {difference:.2f}% under the actual decommission time ({decommission_time} years).'
        else:
            results = f'The predicted decommission time is {y_pred:.2f} years which is {difference:.2f}% over the actual decommission time ({decommission_time} years).'
    else:
